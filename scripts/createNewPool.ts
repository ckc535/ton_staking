...
export async function run(provider: NetworkProvider) {
    ...
    const result = await client.runMethod(Address.parse('EQBFMfdztpl3Oagjyb7pnmCtKORFxtxMXKEtE0osA3Xmv6k2'),"get_wallet_address",[{type: "slice",cell: beginCell().storeAddress(poolsAdminAddress).endCell() }])
    ...
    await jettonWallet.sendDeployNewPool(provider.sender(), poolsAdminAddress, jvtWalletAddress, toNano('10'))
}